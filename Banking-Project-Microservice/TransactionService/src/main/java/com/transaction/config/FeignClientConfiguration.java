 ...
 * This ensures TransactionService can call NotificationService (e.g., Feign fallback on Kafka publish failure).
 */
@Configuration
public class FeignClientConfiguration {
        ...
        synchronized (this) {
            ...
            if (resp.getStatusCode().is2xxSuccessful() && resp.getBody() != null) {
                ...
                if (at != null) {
                    ...
                    System.out.println("Transaction Service: Obtained service-to-service token from Keycloak.");
                    return cachedToken;
                }
            }
            throw new IllegalStateException("Transaction Service: Failed to obtain service token: " + resp.getStatusCode());
        }
    }
    ...
    public RequestInterceptor requestInterceptor() {
        return requestTemplate -> {
            ...
            // Try inbound Authorization header (from the current HTTP request)
            try {
            ...
            // Fallback: client_credentials token for service-to-service
            if (authHeader == null || !authHeader.startsWith("Bearer ")) {
                try {
                    String token = obtainServiceToken();
                    authHeader = "Bearer " + token;
                } catch (Exception ex) {
                    System.out.println("Transaction Service: Failed to obtain service token for Feign call: " + ex.getMessage());
                }
            }
