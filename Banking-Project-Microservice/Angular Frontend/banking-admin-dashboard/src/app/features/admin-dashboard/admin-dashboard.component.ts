...
export class AdminDashboardComponent implements OnInit {
    ...
    updateKycStatus(userId: string, newStatus: KycStatus, adminComment?: string): void {
        this.errorMessage = null;
        this.successMessage = null;
        ...
        // Uncomment for real API call
        const payload: any = { kycStatus: newStatus };
        if (adminComment) {
    ...
    rejectKyc(userId: string): void {
        ...
        if (confirm(`Are you sure you want to REJECT KYC for ${user?.username}?`)) {
            const adminComment = prompt('Please enter a reason for rejection (optional):') || undefined;
            this.updateKycStatus(userId, KycStatus.REJECTED, adminComment);
        }
    }
