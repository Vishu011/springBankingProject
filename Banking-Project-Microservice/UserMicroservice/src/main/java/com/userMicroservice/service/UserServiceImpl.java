...
public class UserServiceImpl implements UserService {
    ...
    public UserResponse updateKycStatus(String userId, KycStatusUpdateRequest request) {
        ...
        try {
            ...
            String msg = "User KYC status changed from " + oldKycStatus + " to " + updatedUser.getKycStatus().name() + ".";
            if (request.getKycStatus() == KycStatus.REJECTED && request.getAdminComment() != null && !request.getAdminComment().isBlank()) {
                msg += " Reason: " + request.getAdminComment();
            }
            publishKycStatusUpdatedEvent(
