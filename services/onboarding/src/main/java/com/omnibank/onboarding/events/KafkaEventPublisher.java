 ...
 * Enabled when onboarding.eventPublisher=kafka (see application.yml).
 * Serializes an event envelope as JSON and publishes to the configured topic.
 */
@ConditionalOnProperty(prefix = "onboarding", name = "eventPublisher", havingValue = "kafka")
@Component
public class KafkaEventPublisher implements EventPublisher {
  ...
  public void publish(String topic, String type, Object payload, String correlationId) {
    ...
    try {
      ...
      // Use correlationId as key if available to keep event ordering per-request; fallback to type
